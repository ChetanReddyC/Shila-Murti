...
export async function GET(
  ...
  // Get customer ID from query or header
  const url = new URL(req.url)
  const customerId = url.searchParams.get('customer_id') || req.headers.get('x-customer-id')
  ...
  if (!token) {
    console.error('[ORDER_DETAILS][TOKEN_ERROR]', 'Failed to generate bridge token')
    return new Response(JSON.stringify({ ok: false, error: 'auth_failed' }), { status: 500 })
  }

  // Fetch the specific order from backend
  const res = await storeFetch(`/store/custom/orders/${orderId}`, { 
    bearerToken: token,
  ...
  if (!res.ok) {
    ...
    console.error('[ORDER_DETAILS][FETCH_ERROR]', { status: res.status, body: text?.slice?.(0, 200) })
